"""微信 WeChatFerry 占位:未装 wcferry 时降级为日志模式,不阻断启动."""
...
class WeChatAdapter(Adapter):
    ...
    async def start(self) -> None:
        if not self.available():
            logger.warning("wcferry 未安装,WeChatAdapter 运行于日志降级模式")
        else:
            logger.info("WeChatAdapter connected")
    ...
    async def send(self, target_id: str, text: str, group_id: str | None = None) -> bool:
        logger.info("WeChat send -> {}|{}: {}", group_id or target_id, target_id, text[:100])
        # TODO: 接入真实 WcFerry send_text
        return True
